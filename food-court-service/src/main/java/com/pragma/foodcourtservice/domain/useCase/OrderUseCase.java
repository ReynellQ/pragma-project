...
import com.pragma.foodcourtservice.domain.spi.IOrderFoodPlatesPersistencePort;
...
public class OrderUseCase implements IOrderServicePort {
    ...
    private final IOrderFoodPlatesPersistencePort orderFoodPlatesPersistencePort;
     ...
     * - Verify if the chef and the restaurant are valid. (They exist and the chef works for the restaurant). In other
     * case return InvalidOrderException.
     * - Verify if the order contains food plates. In other case return InvalidOrderException.
     * - Verify if all the food plates belongs to the restaurant. If a food plate doesn't exist, return
    ...
    public void saveOrder(Order order, List<OrderFoodPlates> foodPlates) {
        ...
        RestaurantEmployee chef = new RestaurantEmployee(idRestaurant, order.getIdChef());
        if(restaurantPersistencePort.doesEmployeeWorksAtRestaurant( chef )){
        ...
        if(client.getIdRole() != ROLES.CLIENT.id){
            throw new NotAClientException();
        }
        order.setIdClient(client.getId());
        order.setDate(LocalDate.now());
        order.setOrderState(ORDER_STATE.PENDING.id);
        orderPersistencePort.saveOrder(order);
        orderFoodPlatesPersistencePort.savesFoodPlatesOrder(foodPlates);
    }
}
